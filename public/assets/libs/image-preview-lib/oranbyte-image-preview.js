(function () {
    function isPreviewable(element) {
        return element?.dataset?.previewable !== 'false';
    }
        ...
        const observer = new MutationObserver(function (mutationsList) {
            for (const mutation of mutationsList) {
                ...
                mutation.addedNodes.forEach(node => {
                    ...
                    const container = node.closest('[data-media-preview="true"]');

                    // Handle the node itself if it's media
                    if (
                        node.matches('img:not([data-previewable="false"]), [data-video-url]:not([data-previewable="false"])') &&
                        isPreviewable(node)
                    ) {
                        mediaViewer.setActionListenerToMedia(node, container);
                    }

                    // Handle nested media elements
                    const medias = node.querySelectorAll(
                        'img:not([data-previewable="false"]), [data-video-url]:not([data-previewable="false"])'
                    );

                    medias.forEach(media => {
                        if (!isPreviewable(media)) return;

                        mediaViewer.setActionListenerToMedia(
                            media,
                            media.closest('[data-media-preview="true"]')
                        );
                    });
                });
            }
...
class OB_MediaViewer {
    ...
    setActionListerToAllMedia() {
        document.querySelectorAll(this.MEDIA_CONTAINER_SELECTOR).forEach(container => {
            const mediaList = Array.from(container.querySelectorAll('img:not([data-previewable="false"]), [data-video-url]:not([data-previewable="false"])'));
            mediaList.forEach(media => {

                this.setActionListenerToMedia(media, container);
            });
        });
    }
    ...
    setActionListenerToMedia(media, container) {
        ...
        const openPreview = () => {
            ...
            this.currentMediaList = Array.from(container.querySelectorAll('img:not([data-previewable="false"]), [data-video-url]:not([data-previewable="false"])'));
