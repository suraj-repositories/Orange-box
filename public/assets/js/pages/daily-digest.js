...
function enableLikeDislike() {
    ...
    likeBtn.addEventListener('click', () => {
        const dailyDigest = likeBtn.getAttribute('data-daily-digest-id');
        const url = authRoute('user.daily-digest.like', { dailyDigest: dailyDigest });
        apiCall(url)
    });

    dislikeBtn.addEventListener('click', () => {
        const dailyDigest = likeBtn.getAttribute('data-daily-digest-id');
        const url = authRoute('user.daily-digest.dislike', { dailyDigest: dailyDigest });
        apiCall(url);
    });
